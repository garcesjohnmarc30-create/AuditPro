              ...
              <div className="mt-1 space-y-1 flex-grow">
                {dayTrips.map((trip) => (
                  <div key={trip.id} className={`p-1 rounded border group relative ${getStatusStyles(trip.status)}`}>
                    ...
                    <button onClick={() => { setRemarksModal(trip); setTempRemarks(trip.remarks || ""); }} className="w-full !bg-transparent text-black text-[8px] p-0.5 mt-1 rounded hover:bg-gray-800">REMARKS</button>
